...
EXPORT_EXTENSIONS: list[str] = [".md", ".json", ".html", ".txt"]
...
def extract_document_text(source: Path | str) -> str:
    """
    Extract markdown text using the Strategy-based extractor.
    """
    global _extractor
...
def extract_document_raw_text(source: Path | str) -> str:
    """
    Extract normalized raw text specifically for classification Tier 2.
    """
    # We read the .txt file generated by AdvancedDoclingExtractor._persist_result
    # or we can modify AdvancedDoclingExtractor to return the full Result object.
    # To keep the API clean, let's assume AdvancedDoclingExtractor.extract returns
    # Markdown, but we can also have a method for raw text.

    source = Path(source)
    txt_path = source.parent / f"{source.stem}.txt"
...
def extract_classification_text(file_path: Path) -> str:
    """Extract text for classification (Tier 2) - now uses raw_text."""
    return extract_document_raw_text(file_path)
